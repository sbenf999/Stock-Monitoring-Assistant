...
class App(superWindow):
    ...
    def weeklyReportUI(self, tab_='Weekly report'):
        ...
        self.generateWeeklyReportButton = customtkinter.CTkButton(self.tabview.tab(tab_), text="Generate weekly report", command=lambda:self.generateWeeklyReport(self.startDatePicker.get_date(), self.endDatePicker.get_date()))
        self.generateWeeklyReportButton.grid(row=4, column=0, padx=(20, 20), pady=20, sticky='w')

        self.reportInfoLabel = customtkinter.CTkLabel(self.tabview.tab(tab_), text=f"*report will generate at {self._appLocation}") #tell the user where the report is generated by getting the working directory
        self.reportInfoLabel.grid(row=4, column=1, padx=(20, 20), pady=20, sticky='w')

    def findDateRange(self, start, stop):        
        dates = []
    ...
    def generateWeeklyReport(self, startDate, endDate):
        if messagebox.askquestion(title='Confirm generate weekly report', message="Do you wish to generate this weekly report?"):
            #ensure both a start and stop date are entered by the user
            if not(len(startDate)==0 or len(endDate)==0):
                #weekly reports are stored in a weekly reports folder, if it doesnt exist then make it
