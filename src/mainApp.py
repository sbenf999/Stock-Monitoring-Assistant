...
class App(superWindow):
    ...
    def recordDeliveryUI(self, tab_='Record a delivery'): #you might want to make this a scrollable fram
        ...
        self.chooseSupplierLabel.grid(row=0, column=0, padx=(20, 20), pady=20)
        self.chooseSupplier = customtkinter.CTkOptionMenu(self.tabview.tab(tab_), dynamic_resizing=False, values=["Value 1", "Value 2", "Value Long Long Long"]) #values list should be taken from a database call once the supplier database is created
        self.chooseSupplier.grid(row=0, column=1, padx=20, pady=20)
        self.enterDeliveryDateLabel = customtkinter.CTkLabel(self.tabview.tab(tab_), text="Enter delivery date:")
        self.enterDeliveryDateLabel.grid(row=1, column=0, padx=(20, 20), pady=20)

        #have 3 boxes for this, aka, day, month, year
        self.deliveryDateEntry = customtkinter.CTkEntry(self.tabview.tab(tab_), placeholder_text="Delivery date xx/xx/xx...")
        self.deliveryDateEntry.grid(row=1, column=1, columnspan=2, sticky="nsew", padx=(20, 20), pady=20)
        self.enterDeliveryTimeLabel = customtkinter.CTkLabel(self.tabview.tab(tab_), text="Delivery time:")
        self.enterDeliveryTimeLabel.grid(row=2, column=0, padx=(20, 20), pady=20)
        ...
        self.enterDeliveryTimeLabelAbs.grid(row=2, column=1, padx=(20, 20), pady=20)
